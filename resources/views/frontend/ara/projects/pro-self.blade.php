        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-6 attribute">
                    <ul>
                        <li><h4> {{$product->aratitle}}</h4></li>
                        <br>
                        <div class="row">
                            <div class="col-md-6">
                                <li><b>قانون : </b>{{$product->sku}}</li>
                                @foreach($product->attributevalue as $attribute)
                                    @if($attribute->attributeGroup->title == "متراژ")
                                        <li><b>منطقة :{{$attribute->aratitle}}</b></li>
                                    @endif
                                    @if($attribute->attributeGroup->araname == "غرفة نوم")
                                        <li><b> غرفة :{{$attribute->aratitle}}</b></li>
                                    @endif
                                @endforeach
                            </div>
                            <div class="col-md-6">
                                <li><b>مدينة : </b>{{$product->aracountry}}</li>
                            </div>
                        </div>
                    </ul>
                    <hr>
        ...
        <div class="container" style="margin-bottom: 5%">
            <div class="characters">
                <ul class="nav nav-tabs" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" data-toggle="tab" href="#home">مواصفات</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#menu1">وصف</a>
                    </li>
                </ul>
                <div class="tab-content">
                    <div id="home" class="container tab-pane active"><br>
                        <table class="table table-striped">
                            ...
                            @foreach($product->attributevalue as $attribute)
                                <tr>
                                    <td>{{$attribute->attributeGroup->araname}}</td>
                                    <td>{{$attribute->aratitle}}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
